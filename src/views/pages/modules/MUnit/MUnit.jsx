...
const MUnit = () => {
  ...
  const TodoGetData = async () => {
    const res = await ApiService.getDataJWT('/mUnit')
  ...
  const TodoDeleteData = async (id) => {
    fireNotif.notifWarning('Delete this item?').then(async (swalRes) => {
      if (swalRes.isConfirmed) {
        ...
        const resAPi = await ApiService.deleteDataJWT('/mUnit', id)
        ...
        menuPrivilage.flag_update || menuPrivilage.flag_delete ? (
          <Box>
            {menuPrivilage.flag_update ? (
              <IconButton
                onClick={() => Navigate('/unit/update', { state: { id: row.original.id } })}
              >
  ...
  return (
    <>
      <CCard className="mb-4">
        <CCardHeader>Master Unit</CCardHeader>
        <CCardBody>
